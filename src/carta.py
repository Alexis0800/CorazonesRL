...
class Carta:
    ...
    @property
    def palo(self) -> int:
        """Retorna el palo de la carta (0=Tréboles, 1=Diamantes, 2=Picas, 3=Corazones)."""
        return self._palo

    @property
    def valor(self) -> int:
        """Retorna el valor numérico de la carta (2-14)."""
        return self._valor

    @property
    def id(self) -> int:
        """Retorna el índice único de la carta (0-51)."""
        return self._id

    # --- Propiedades de conveniencia para reglas de Corazones ---

    @property
    def es_corazon(self) -> bool:
        """True si la carta es de corazones (palo 3)."""
        return self._palo == 3

    @property
    def es_dama_de_picas(self) -> bool:
        """True si la carta es la Dama de Picas (palo 2, valor 12)."""
        return self._palo == 2 and self._valor == 12

    @property
    def puntos(self) -> int:
        """Puntos que vale esta carta según las reglas de Corazones.

        Returns:
            1 si es corazón, 13 si es la Dama de Picas, 0 en otro caso.
        """
        if self._palo == 3:
            return 1
    ...
    def es_dos_de_treboles(self) -> bool:
        """True si la carta es el 2 de Tréboles (palo 0, valor 2)."""
