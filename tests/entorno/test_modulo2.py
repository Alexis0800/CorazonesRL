...
class TestActionMasking:
    ...
    def test_action_masks_primer_turno_solo_dos_treboles(self):
        """En el primer turno, las acciones legales son subconjunto de cartas en mano."""
        from src.entorno.single_agent import CorazonesEnv
        env = CorazonesEnv()
        obs, _ = env.reset()
        mask = env.action_masks()
        ...
        # Siempre debe haber al menos una acción legal
        assert num_legales >= 1
        # Las acciones legales deben corresponder a cartas en mano
        for i in range(52):
            if mask[i]:
                assert mano[i] == 1.0, (
